...
/// Accepts 32-128 URL-safe characters (letters, digits, `-`, `_`) generated by the device.
...
/// Signs a guest in with the secret stored on the device.
///
/// The same `guest_unique_id` always resolves to the same user, so calling this again is
/// also how a device renews its short lived token. Only an unknown id creates a user and
/// that creation is limited per client network.
pub async fn handle(
    state: &AppState,
    input: &GuestInputDto,
